...
from _3_compression_common import (
    ...
    allocate_layerwise_ranks_exact_budget,
    alpha_tag,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Uniform-baseline rounding rule used to define the exact total rank budget.",
    )
...
def main() -> None:
    ...
    exact_total_budget = int(uniform_budget["total_budget"])
    layerwise_ranks, rank_details = allocate_layerwise_ranks_exact_budget(
        activation_rows=activation_rows,
        layer_indices=layer_indices,
        alpha=args.alpha,
    ...
    rank_detail_by_layer = {int(entry["layer_index"]): entry for entry in rank_details}
    ...
    with torch.no_grad():
        for layer_index in tqdm(layer_indices, desc="Compressing layers", unit="layer"):
            ...
            target_rank = int(layerwise_ranks[layer_index])
            raw_rank = float(rank_detail_by_layer[layer_index]["raw_rank"])
                ...
                plan_rows.append(
                    {
                        ...
                        "compression_method": "activation_aware_layerwise",
    ...
    write_json(
        meta_path,
        {
            ...
            "uniform_baseline_rank": int(uniform_budget["uniform_rank"]),
            "exact_total_rank_budget_per_module": exact_total_budget,
            "actual_total_rank_budget_per_module": int(sum(layerwise_ranks.values())),
            "eigenvalue_floor": args.eigenvalue_floor,
            "layerwise_ranks": layerwise_ranks,
